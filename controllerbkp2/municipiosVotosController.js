...
const syncMunicipiosVotos = async (req, res) => {
    ...
    try {
        ...
        if (!Array.isArray(candidatos)) {
            console.warn('API response "candidatos" is not an array:', apiResponse.data);
        ...
        for (const candidato of candidatos) {
            ...
            if (candidato_codigo === undefined || partido_id === undefined || currentVotos === undefined) {
                console.warn('Skipping record due to missing data:', candidato);
                continue;
            }
            ...
            try {
                ...
                console.error(`Error fetching previous votes for candidate ${candidato_codigo}:`, dbError);
            }
            ...
            if (currentVotos !== previousVotos) {
                ...
                try {
                    ...
                    console.error(`Error logging vote update for candidate ${candidato_codigo}:`, dbError);
                }
            }
        }

        res.status(200).json({ message: 'Data sync process completed for municipio.', changes_logged: changesCount });

    } catch (error) {
        console.error('Error syncing municipal votes:', error.message);
        if (error.response) {
            console.error('API Error Response:', error.response.data);
            return res.status(error.response.status).json({ message: 'Error from external API.', details: error.response.data });
        }
